<template>
	<div class="bgView" @dblclick="originShow=true">
		<div :class="['json-view', length ? 'closeable' : '']" :style="'font-size:' + fontSize+'px'" v-if="!originShow">
		...
		<div v-if="originShow">
			<el-input type="textarea" v-model="originJson" rows="8" @blur="oriToJsonView"></el-input>
		</div>
	</div>
</template>
	...
	export default {
		...
		data() {
			return {
				innerclosed: true,
				originShow: false,
				originJson: JSON.stringify(this.json),
			};
		},
		methods: {
		...
		computed: {
			isArray() {
				return (
					Object.prototype.toString.call(this.json) === "[object Array]"
				);
			},
			length() {
				return this.isArray
